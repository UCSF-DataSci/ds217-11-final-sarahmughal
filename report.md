...
This project analyzes 196,279 hourly weather observations collected between April 2015 and December 2025 from three Chicago beach weather stations. The goal was to uncover temporal weather patterns and develop predictive models for Air Temperature using a structured nine-phase data science workflow. After thorough cleaning, wrangling, feature engineering, and model building, three models were evaluated, with XGBoost performing the strongest. It achieved a test R² of 0.7897, RMSE of 4.65°C, and MAE of 2.84°C, driven largely by the high predictive value of Wet Bulb Temperature. Overall, the results reveal clear seasonal cycles, steady weekly humidity patterns, and consistent relationships among key meteorological variables.

## Phase-by-Phase Findings

### Phase 1-2: Exploration

Initial exploration showed that the dataset contains 196,279 rows and 18 columns of hourly observations collected from three Chicago lakefront stations: 63rd Street, Foster, and Oak Street. The dataset captures a wide range of meteorological variables including Air Temperature, Wet Bulb Temperature, Humidity, Solar Radiation, Battery Life, and detailed wind, rain, and barometric measurements. These observations span more than a decade, from April 25, 2015 through December 2, 2025.
...
While the workflow produced strong predictive results, several limitations remain. The dataset had substantial missingness in precipitation-related variables, and imputing zeros—though reasonable—may have oversimplified true weather patterns. Outlier capping also restricted some extreme but potentially valid measurements, especially in Solar Radiation. The dataset came from only three stations, which limits how broadly the findings can be generalized. Some predictors, such as Total Rain and Interval Rain, were highly correlated, and the feature set included only a modest number of temporal features. From a modeling standpoint, Linear Regression struggled with the nonlinear structure of the data, and XGBoost, while effective, carries some risk of overfitting.

Future work could expand the dataset to include more stations and additional environmental variables to improve both coverage and model robustness. Creating richer time-based features—such as lags or multiple rolling windows—may help capture short- and long-term weather dynamics more accurately. Exploring additional models, tuning Random Forest more deeply, or trying simple ensemble approaches could further improve predictive performance. These steps would help refine the model and strengthen its ability to generalize to new conditions.
